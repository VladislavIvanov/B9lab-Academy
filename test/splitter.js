...
contract('Splitter', function(accounts) {
    ...
    it("Should allow owner to terminate contract", async () => {
        let initBalance = web3.eth.getBalance(ownerAccount);
        
        let txObject = await splitter.closeContract({ from: ownerAccount });
        assert.equal(txObject.logs.length, 1, "Did not log ContractDesctroyed event");
        let logEvent = txObject.logs[0];        
        assert.equal(logEvent.event, "ContractDestroyed", "Did not ContractDestroyed");
        assert.equal(logEvent.args.destroyer, ownerAccount, "Did not selfdescructed by owner");

        let newBalance = web3.eth.getBalance(ownerAccount);
